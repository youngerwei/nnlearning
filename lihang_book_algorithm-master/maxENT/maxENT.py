...
'''
2类分类器，只对0和1进行分类。
首先假设所有像素点代表为0和为1的概率相等 --- 最大熵模型
然后通过约束条件计算其条件熵。约束条件具体为：
1、将图像每个像素点的索引和像素值组成一个字符串，构建一个一个这样的字符串列表a，非常长，768*42000*0.67多副图像
2、合并a中相同的字符串，也就是统计相同位置相同像素的出现的次数；
   同时统计各个数字label下所有图像的相同位置相同像素的出现的次数。
'''
...
class MaxEnt(object):
    ...
    def cal_Pxy_Px(self, X, Y):
        self.Pxy = defaultdict(int)     #(x,y)的联合概率
        self.Px = defaultdict(int)      #x的概率
    ...
    def cal_EPx(self):
        '''
        计算书83页最上面那个期望，特征函数f(x,y)关于模型p(Y|X)与经验分布p(X)的期望值
        '''
