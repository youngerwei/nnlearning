...
'''
1、将数据分为训练数据和测试数据；
2、从测试数据中取一个a，从训练数据中取10个b，计算a到b的欧氏距离；
3、然后将所有的训练数据针对a计算欧式距离c，如果距离小于前面10个数据中最近的那个b（x），就用c替换b（x）；
4、这样得到的是训练数据中和测试数据距离最近的10个数据，根据这10个数据的标签确定测试数据的标签。
   在理想情况下，这10个标签会是同一个数。
'''
import pandas as pd
import numpy as np
...
def Predict(testset, trainset, train_labels):
    ...
    for test_vec in testset:
        ...
        # 先将前k个点放入k个最近邻居中，填充满knn_lis
        for i in range(k):
            label = train_labels[i]
            train_vec = trainset[i]

            # 计算两个点的欧氏距离
            # Calculate the distance between the current test point and the first 10 training points
        ...
        for i in range(k, len(train_labels)):
            ...
            # 计算两个点的欧氏距离
            # Calculate the distance between the rest training point to the test point
            dist = np.linalg.norm(train_vec - test_vec)
            ...
            # Only change the furthest point, even if there are several points further than current points
            if dist < max_dist:
